...
void get_stack_args(char *file_name, void **esp, char **save_ptr)
{
    int arg_count = 0;
    int tot_length = 0;
    char *token = file_name, *args_pointer;
    ...
    *args_pointer = (char *) stack_pointer;

    /* Align the stack pointer to a 4-byte boundary */
    int word_align = (4 - (tot_length % 4)) % 4;
    if (word_align)
    {
    ...
    /* Push address of argument addresses */
    ...
    *esp = stack_pointer;
    hex_dump((int)*esp, *esp, tot_length*4, 1);
}
